...
import { buildInvestigationCoachPrompt } from "@/lib/ai/tutorPromptTemplates";
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
                        ...
                        <div key={q.id}>
                          {!asked ? (
                            <button
                              ...
                              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
                            >
                              {q.question}
                            </button>
                          ) : (
            ...
            {!isTerminal && (
              <Card>
                <div className="mb-1 flex flex-wrap items-start justify-between gap-3">
                  ...
                  <AskTutorLink params={{ mode: "investigation-coach", scenario: scenario.id, prompt: buildInvestigationCoachPrompt(scenario) }} variant="button">
                    Ask Tutor (Coach)
                  </AskTutorLink>
                </div>
                <div className="space-y-2">
                  {node.actions.map((action, i) => (
                    <button
                      ...
                      className="group flex w-full items-start gap-3 rounded-xl border border-slate-200 p-3 text-left transition-all duration-200 hover:-translate-y-0.5 hover:border-blue-300 hover:bg-blue-50/50 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/40 dark:border-slate-800 dark:hover:border-blue-800/70 dark:hover:bg-blue-950/20"
                    >
                      <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-100 text-xs font-semibold text-slate-500 transition-colors duration-200 group-hover:bg-blue-600 group-hover:text-white dark:bg-slate-800 dark:text-slate-400">
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              className="w-full rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
            >
              Restart scenario
            </button>
          </div>
